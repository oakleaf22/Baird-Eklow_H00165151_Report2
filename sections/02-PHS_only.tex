...
\subsection{Pumped hydro storage} \label{sec:PHS}
...
PHS schemes can store large volumes of water, with storage periods that vary from typically hours to days and even years \citep{Chen2009}.
The power rating of PHS is the highest compared to other forms of EES; this makes it suitable for frequency control and provision of reserve electricity \citep{Chen2009}.
...
However, PHS can also work well to compensate for the intermittent nature of RE generation \citep{Kocher-Oberlehner2014}.

The efficiency of a PHS scheme is considered to be between 75\% and 95\%, depending on the source.
It depends on losses due to things such as electricity conversion, friction in the conduits, and pump and turbine efficiencies.
It can also vary due to aspects such as precipitation and the duration of water storage (which can result in small evaporation losses or leaks into the groundwater) \citep{WillsComment2015, Kocher-Oberlehner2014, Chen2009}.
...
It is believed that these figures depend on the system efficiencies the sources are considering.
...
continuously for around 20 hours [before the reservoir is depleted] if necessary" \citep{ScottishPowernd, Mearns2018}.
...
Hence, the figures in Table~\ref{tbl:existing_phs} have been estimated based on the Cruachan findings.
...
Research shows that investments in new PHS schemes in Scotland is waning due to strong planning and environmental opposition and a scarcity of available sites for two large reservoirs and one or two dams \citep{Strathclyde2004, ScotsRenewables2011, Chen2009}.
...
Figure~\ref{fig:map_potential_PHS} shows 13 locations in Scotland with potential for PHS, most of which already have a hydroelectric facility \citep{MacKayDavid2009}.
...
If Loch Sloy's dam were raised by another 40~m then the energy storage capacity could be raised to 40~GWh \citep{MacKayDavid2009}.
...
Scotland has potential to store well over one week's worth of power and energy demand according to Mearns' method in Question~1 (i.e. 1.43~GW/ 240~GWh).
...
The capital cost of building a PHS plant is dependent on the energy storage capacity of the reservoirs and the power rating \citep{Zach2012}.
The can range from as low as \$500/kWh to \$8,000/kWh \citep{Kocher-Oberlehner2014}.
In terms of capacity, this can range from 450~\euro{}/kW to 2,500~\euro{}/kW \citep{Zach2012} or from \$1,000/kW to \$6,000/kW \citep{Kocher-Oberlehner2014}.
Being a mature technology, PHS's capital cost is not expected to change substantially in the future \citep{Zach2012}.
The minimum and maximum costs found are presented in Pound Sterling in Table~\ref{tbl:costs_kW_kWh}

\input{tables/costs_kW_kWh}

While the capital cost is a long-term investment with a typical payback time of 20 to 40~years, it is worthy to consider that operational and maintenance (O\&M) costs are low and the typical lifetime of a PHS plant ranges from 40 to 100 years, or even more \citep{Kocher-Oberlehner2014, Zach2012, Chen2009}.
O\&M costs could not be found for PHS, but are presumed to be similar to O\&M costs for conventional hydroelectricity plants which range from 1.5\% to 2.5\% of the investment costs per year \citep{Lako2010, IRENA2012}.
The large variations in cost and payback time correlates with variations arising from differences in site design, labour costs and amount and type of materials required \citep{Kocher-Oberlehner2014}.
%The costs of dams can be averted through the redevelopment of existing hydroelectric plants into PHS.
...
Constructing and deploying a PHS scheme entails some major constraints, including a high capital cost and an approximate lead in time of ten years.
There are also environmental issues such as removing trees and vegetation from the large amounts of land required by the reservoir \citep{Chen2009}.
Other design considerations include the length of the waterways; long waterways are costly and result in higher hydraulic losses.
There is also the transmission distance to demand; the shorter this distance, the better because this simultaneously reduces transmission integration costs and transmission losses.
A scheme located in the vicinity of generation centres and near the main national grid is thus ideal \citep{Louwinger2008}.
Furthermore, building in a rural or remote location might incur additional costs in peripheral infrastructure, e.g. access roads to the site.

Table~\ref{tbl:costs_calc} presents the range of capital costs for building all the required electricity storage in Mearns' scenario using PHS only.
Since the power costs are so small compared to the storage costs, it is assumed the former are included in the latter.
To provide three days' amount of storage, Scotland would need to invest between £34.7~billion and £558.3~billion.
Costs naturally increase if building for more storage, up to £1.4~trillion for one week's amount of storage.
Considering that Scotland's GDP (gross domestic product) was £182.3~billion in 2018 \citep{ScottishGov2018}, it is possible that it would be too costly to build all this PHS (at least all in one go).
It might require several decades or more than a century to build, considering the payback time.

However, as said, these options would not meet Scotland's peak demand of 3.43~GW, which Scotland does not even seem to have the PHS potential for according to Table~\ref{tbl:potential_phs}.
Theoretically, to meet this demand, the costs could be in the range of £1.0~billion to £12.3~billion, but that just considers the cost per kW.
It is likely the amount of storage required will still be in the range of 103~GWh to 240~GWh, as calculated with Mearns' method, to meet the mean winter demand.
The storage capacity (in GWh) is likely to increase the costs significantly, likely in the same range as presented in Table~\ref{tbl:costs_calc}.

%\hlc{Mention reduction in costs due to existing hydroelectric dam infrastructure, but likely increase due to rural/ remote locations.}

\input{tables/costs_calc}
